...
from src.exceptions import DataNotFoundError, ModelLoadError
...
if __name__ == '__main__':
    ...
    from src import config as config_main
    from src.exceptions import DataNotFoundError
    from src.utils import ensure_dir_exists as ensure_dir_exists_main
    ...
    parser.add_argument("--processed-path", type=str, default=config_main.PROCESSED_DATA_PATH,
                        help="Path to the processed Parquet data file.")
    parser.add_argument("--embeddings-path", type=str, default=config_main.EMBEDDINGS_PATH,
                        help="Path to save the embeddings .npy file.")
    parser.add_argument("--metadata-path", type=str, default=config_main.EMBEDDING_METADATA_PATH,
                        help="Path to save the embedding metadata JSON file.")
    parser.add_argument("--model-name", type=str, default=config_main.EMBEDDING_MODEL,
                        help="Name of the sentence-transformer model to use.")
    parser.add_argument("--no-progress-bar", action="store_false", dest="show_progress_bar",
                        help="Disable the progress bar during embedding generation.")
    parser.add_argument("--batch-size", type=int, default=config_main.DEFAULT_BATCH_SIZE,
